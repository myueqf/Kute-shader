...
vec3 blur(sampler2D image, vec2 texcoord, vec2 direction, float Scale, float Offset) {
    ...
    color += texture2D(image, texcoord).rgb * 0.27;             // soften center weight a little
    if (clamp(texcoord + off1 * 2, BoundLower, BoundUpper) == texcoord + off1 * 2)
        color += texture2D(image, texcoord + off1).rgb * 0.365; // slightly stronger side samples for gentle spread
    if (clamp(texcoord - off1 * 2, BoundLower, BoundUpper) == texcoord - off1 * 2)
        color += texture2D(image, texcoord - off1).rgb * 0.365;
    return color;
}
...
vec3 blur3x3(sampler2D image, vec2 texcoord) {
    ...
    Color += texture2D(image, texcoord).rgb * 0.22;              // soften center weight a touch
    Color += texture2D(image, texcoord + Off1).rgb * 0.195;
    Color += texture2D(image, texcoord - Off1).rgb * 0.195;
    Color += texture2D(image, texcoord + Off2).rgb * 0.195;
    Color += texture2D(image, texcoord - Off2).rgb * 0.195;
    return Color;
}
