...
float water_fog() {
    ...
    return smoothstep(0, 56, TerrainDepth - ScreenDepth) * (WATER_FOG_STRENGTH * 0.7); // softer fog, longer range
}

vec3 tint_underwater(vec3 FinalColor) {
    if (isEyeInWater == 1) {
        vec3 WaterColor = to_linear(vec3(f_WATER_RED * 1.1, f_WATER_GREEN * 1.15, f_WATER_BLUE * 1.2)); // lighter, pastel water tint
        WaterColor = mix_preserve_c1lum(WaterColor, fogColor, f_BIOME_WATER_CONTRIBUTION * 0.7);
        float DistFromSurface = 1 - eyeBrightnessSmooth.y / 260.; // softer distance fade
        vec3 Tinted = WaterColor * DistFromSurface;
        FinalColor = mix_preserve_c1lum(FinalColor, WaterColor, DistFromSurface * 0.9);
    }
    return FinalColor;
}

float schlick(vec3 V, vec3 N) {
    const float R = 0.07; // softer fresnel reflectivity
...
vec3 get_water_normal(vec2 Coords, vec3 WorldNormal) {
    if (abs(WorldNormal.y) < 0.99) {
        Coords -= frameTimeCounter * normalize(WorldNormal.xz) * 2.5; // slower normal animation
    }
    vec2 N = noise_water(Coords);
    return normalize(vec3(N.x * 0.8, N.y * 0.8, 1 - (N.x * N.x + N.y * N.y) * 0.7)); // softer normals, less sharp
}

vec3 sky_reflection(vec3 ReflectedVec, float WNy, float Dist) {
    #ifdef DIMENSION_OVERWORLD
        vec3 SunGlare = get_sun_glare(Dist) * 0.85; // dimmer glare
        vec3 Reflection = get_sky(ReflectedVec, SunGlare);
        #ifdef REFLECT_SUN
            if (WNy > 0.01) { 
                Reflection.rgb += round_sun(Dist) * 2.5 * isOutdoorsSmooth; // gentler sun reflection
            }
        #endif
        return Reflection;
    #else
        return fogColor.rgb * 0.9; // slightly softer fog color
    #endif
}
...
vec3 ssr(vec3 RVec, float Dist, vec3 ViewPos, float Fresnel, float WNy, bool IsDH) {
    ...
    for (int i = 1; i <= SSR_STEPS; i++) {
        ...
        if (RealDepth < 0.58) { // softer threshold
            break;
        }

        if (ExpectedPos.z > RealDepth) {
            if (ExpectedPos.z - RealDepth > Offset.z * 3.5) {
                break;
            }

            for (int j = 1; j <= int(round(Fresnel * 2.5)); j++) {
                Offset /= 2;
            ...
            return texture2D(gaux1, ExpectedPos.xy).rgb * 0.95; // slight dimming for softness
        }
        ExpectedPos += Offset;
    }
...
vec3 flipped_image_ref(vec3 RVec, float Dist, vec3 ViewPos, float WNy, bool IsDH) {
    ...
    float Offset = min(1100, 55 + dhRenderDistance / 5);
    #else
    float Offset = 55 + far / 5;
    #endif
    ...
    if(SamplePos.xy == clamp(SamplePos.xy, 0, 1)) {
        ...
        if(SamplePos.z < 1 && SamplePos.z > 0.58 && RealDepth < SamplePos.z) {
            SamplePos.z = RealDepth;
            vec3 ViewPosReal = to_view_pos(SamplePos, IsDH);
            if(len2(ViewPosReal) + 35 > len2(ViewPos)) {
                return texture2D(gaux1, SamplePos.xy).rgb * 0.9; // softer reflection
            }
        }
    }
    return sky_reflection(RVec, WNy, Dist);
...
vec4 get_fancy_water(vec3 ScreenPos, vec3 ViewPos, vec4 BaseColor, float SkyBrightness, mat3 TBN, bool IsDH) {
    ...
    vec3 PlayerPos = to_player_pos(ViewPos);
        ...
        if (WorldNormal.y > -0.02) { // more tolerant threshold
            #if REFLECTIONS == 1
            ...
            #endif
            BaseColor.rgb = mix(BaseColor.rgb, Reflection, Fresnel * 0.9);
        }
    #endif
